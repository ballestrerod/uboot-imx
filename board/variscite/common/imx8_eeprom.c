...
static int var_scu_eeprom_read(uint8_t *buf)
{
	uint32_t p1, p2;
	sc_err_t ret = 0;
	sc_ipc_t ipc_handle;
	...
	ret = sc_misc_board_ioctl(ipc_handle, &p1, &p2, (uint32_t *)&buf);
...
int var_scu_eeprom_read_header(struct var_eeprom *e)
{
	...
	ret = var_scu_eeprom_read(eeprom_buf);
	if (ret) {
		debug("SCU EEPROM read failed\n");
		return ret;
	}

	debug("var_eeprom_read_header: copying %lu bytes from %p to %p\n",
		sizeof(*e), e, eeprom_buf);

	memcpy(e, eeprom_buf, sizeof(*e));
